...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Generate seed CoT reasoning traces using VLM")
    parser.add_argument("--meta_json", type=str, default="/dataset/rvos_root/ReVOS/meta_expressions_train_select.json",
                        help="Path to the meta expressions JSON file")
    parser.add_argument("--mask_dict", type=str, default="/dataset/rvos_root/ReVOS/mask_dict.json",
                        help="Path to the mask dict JSON file")
    parser.add_argument("--data_path", type=str, default="/dataset/rvos_root/ReVOS",
                        help="Root path of the ReVOS dataset")
    parser.add_argument("--output_json", type=str, default="/dataset/rvos_root/ReVOS/video_metas_subset_6k_.json",
                        help="Path to save the output JSON file")
    parser.add_argument("--max_samples_per_video", type=int, default=10,
                        help="Maximum number of expression samples per video")
    ...
    video_meta_dict = json.load(open(args.meta_json, 'r'))["videos"]
    ...
    num_sub_processor = args.num_workers
    ...
    print(time.time() - start_time)                 1. Briefly describe what is happening across all frames in the video.        \
                 2. Justify why the frame at timestamp {timestamps[keyframe_id]} best represents the described target.      \
                 3. Describe what the target is doing and precisely where it is located in the selected frame.        \
                 **Important:**         \
                 - You must analyze the provided keyframe '{timestamps[keyframe_id]}' — do not choose a different one.       \
