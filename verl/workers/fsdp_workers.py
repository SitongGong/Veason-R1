...
class FSDPWorker(Worker):
    def __init__(
        ...
        self._is_sam2_reward = role == "sam2_reward"
        
        # 如果是SAM2专用worker，禁用其他功能
        if self._is_sam2_reward:
    ...
    def _build_model_optimizer(
        ...
        if self._is_actor or self._is_critic:       # build optimizer and lr scheduler
    ...
    def init_model(self):
        ...
        # 在所有主要模型初始化完成后，初始化SAM2（如果需要）
        if self._is_sam2_reward:
            self.print_rank0("Initializing SAM2 for reward computation...")
            sam2_success = self.init_sam2_model()
            if not sam2_success:
                self.print_rank0("SAM2 initialization failed")
    ...
    def get_sam2_predict(self, image, point_coords=None, point_labels=None, box=None):
        """获取SAM2预测器"""
        if not self._sam2_initialized or self.rank != 0:
            return None
        try:
            # import pdb
            # pdb.set_trace()
            ...
            torch.cuda.empty_cache()
            # log_gpu_memory_usage("After compute_mask_prediction")
            ...
            self.print_rank0(f"SAM2 prediction failed: {e}")
            return None
    ...
    def get_sam2_multiple_predict(self, image, boxes=None):
        ...
        try:
            ...
            # 这里需要预测多个bbox所对应的mask
            masks = np.zeros(image.size, dtype=np.uint8)
            for box in boxes:
                ...
                sorted_ind = np.argsort(score)[::-1]      # 将预测的iou分数由大到小排列
                mask = mask[sorted_ind]
                mask = mask[0].astype(np.uint8)      # 取出iou分数最大的值
                
                masks = masks | mask     # 将多个bbox所预测的mask取并集得到最终的masks
    ...
    @register(dispatch_mode=Dispatch.ONE_TO_ALL) 
    def get_sam2_video_predict(self, video_frame_list, boxes=None, keyframe_idx=None):
        if not self._sam2_initialized or self.rank != 0:
            return None
        try:
            # pdb.set_trace()
            state = self.sam2_predictor.init_state(video_frame_list)
            self.sam2_predictor.reset_state(state)
            # 根据关键帧索引分割出关键帧中的目标
            for num, box in enumerate(boxes):
                frame_idx, object_ids, masks = self.sam2_predictor.add_new_points_or_box(state, 
            ...
            for frame_idx, frame_segment_dict in sorted_video_segments.items():
                ...
                for obj_id, mask_logits in frame_segment_dict.items():
                    frame_mask = frame_mask | mask_logits.astype(np.uint8)
            ...
            return {"video_masks": video_masks}
    ...
    @register(dispatch_mode=Dispatch.ONE_TO_ALL)
    def init_sam2_model(self, mode="images"):
        """在FSDPWorker中初始化SAM2模型"""
        try:
            # 使用当前CUDA设备
            sam2_device = f"cuda:{torch.cuda.current_device()}"
            
            self.print_rank0(f"Initializing SAM2 on device: {sam2_device}")
            log_gpu_memory_usage("Before SAM2 init")
            
            # SAM2配置
            checkpoint_path = "sam2_large/sam2_hiera_large.pt"
            ...
            if self.rank == 0:
                ...
                if mode == "images":
                    ...
                    self.sam2_model = build_sam2(model_cfg, checkpoint_path, device=sam2_device)
                    self.sam2_model.eval()
                    for param in self.sam2_model.parameters():     # 禁用梯度计算
                    ...
                    self.sam2_predictor = build_sam2_video_predictor(model_cfg, checkpoint_path, device=sam2_device)
                    for param in self.sam2_predictor.parameters():
                        param.requires_grad = False
                    
                self._sam2_initialized = True
                
                torch.cuda.empty_cache()
                log_gpu_memory_usage("After SAM2 init")
                self.print_rank0("SAM2 model initialized successfully")
            
            return True
            
        except Exception as e:
            self.print_rank0(f"Failed to initialize SAM2: {e}")
            return False
    ...
    def update_actor(self, data: DataProto, tokenizer=None):
    ...
    @register(dispatch_mode=Dispatch.DP_COMPUTE_PROTO)
    def compute_log_prob(self, data: DataProto, tokenizer=None):
        assert self._is_actor
        if self._use_param_offload:
            load_fsdp_model(self.fsdp_module)
        ...
        with self.ulysses_sharding_manager:
            ...
            output, entropys, self_certaintys = self.actor.compute_log_prob(data=data, tokenizer=self.tokenizer)
    ...
    @register(dispatch_mode=Dispatch.DP_COMPUTE_PROTO)
    def compute_ref_log_prob(self, data: DataProto, tokenizer=None):
        assert self._is_ref
        if self._use_param_offload:
            load_fsdp_model(self.fsdp_module)
        ...
        with self.ulysses_sharding_manager:
            data = self.ulysses_sharding_manager.preprocess_data(data)
            output, _, _ = self.ref_policy.compute_log_prob(data=data, tokenizer=self.tokenizer)
