...
class DataParallelPPOActor(BasePPOActor):
    def __init__(
        ...
        self.compute_self_certainty_from_logits = torch.compile(verl_F.self_certainty_from_logits, dynamic=True)

    def _forward_micro_batch(
        self, micro_batch: Dict[str, torch.Tensor], temperature: float, tokenizer=None
        ...
        if self.config.padding_free:
            ...
            self_certainty = verl_F.reasoning_certainty_from_logits(logits, responses, tokenizer)      # bsz
    ...
    @torch.no_grad()
    def compute_log_prob(self, data: DataProto, tokenizer=None) -> torch.Tensor:
        ...
        for micro_batch in tqdm(micro_batches, desc="Compute log probs", disable=(self.rank != 0)):
            ...
            entropy, log_probs, self_certainty = self._forward_micro_batch(model_inputs, temperature=temperature, tokenizer=tokenizer)
    ...
    def update_policy(self, data: DataProto, tokenizer=None) -> Dict[str, Any]:
        ...
        for i, mini_batch in enumerate(mini_batches):
            ...
            for micro_batch in tqdm(micro_batches, desc=f"Update policy [{i + 1}/{n}]", disable=(self.rank != 0)):
                ...
                # pdb.set_trace()
                entropy, log_prob, _ = self._forward_micro_batch(model_inputs, temperature=temperature, tokenizer=tokenizer)
