...
class ReVOS_KeyframeSFT_Dataset(Dataset):
    """
    ...
    def __init__(
        ...
        self.user_prompt =  "Reference Video Frames: <video> " \
                            "You are given {num} reference frames uniformly sampled from a 1 FPS video. Each image is preceded by its timestamp." \
                            "Given the following object description: '{Question}', please complete the following steps: " \
                            "1. Carefully analyze the visual content across all frames and **select exactly one keyframe** where the referred object(s) are most clearly visible. (i.e. the frame whose object mask / visible area is maximal or is best aligned with the description)" \
                            "2. In the selected keyframe, **predict the 2-D bounding box(es)** of all referred object instances. " \
                            "Output the reasoning step-by-step in <think> </think> and final answer in <answer> </answer> tags. " \
                            "Output the bbox(es) of the referred object in JSON format. " \
