...
class ReVOS_KeyframeSFT_Base_Dataset(Dataset):
    """
    ...
    def __init__(
        ...
        train_json_path = os.path.join(data_path, "meta_expressions_train_select.json")
        ...
        for video_name, vid_data in video_data.items():
            ...
            for exp_id, exp_dict in vid_data["expressions"].items():
                if exp_dict["type_id"] == 2:     # 暂时不对物体不存在的情况进行设计
                    continue
                meta = {}
    ...
    def __getitem__(self, index):
        ...
        for seg_frame_id in frame_ids:
            ...
            for x, obj_id in zip(anno_ids, obj_ids):  # 对于每个物体的标注索引和类别索引
                ...
                m_final_.append(m_object)
            
            m_final_list.append(np.stack(m_final_, axis=0))      # num_obj, H, W
            m_final_list_.append(m_final)

        m_final_list = np.stack(m_final_list, axis=0)  # (num_frame, num_obj, H, W)
        m_final_list_ = np.stack(m_final_list_, axis=0)  # (num_frame, H, W)
        
        # print(m_final_list.shape)
        # print(m_final_list_.shape)

        # # 这里改一下，每次从面积最大的前5帧中随机抽取一帧
        # areas = (m_final_list_ > 0).sum(axis=(1, 2))       # shape (T,)
        # # print(areas)
        # if self.rvos_sampled_frames < video_length:
        #     # ==== 1. 从前 %n 帧中找 mask 最小的帧 ====
        #     percent_n = 1 / self.rvos_sampled_frames  # 前 20%
        #     head_n = max(1, int(video_length * percent_n))
        #     head_indices = list(range(head_n))
        #     first_frame = min(head_indices, key=lambda i: areas[i])  # mask 最小的一帧

        #     # ==== 2. 从 mask 最大的前5帧中随机选一帧作为关键帧 ====
        #     # top_k_indices = areas.argsort()[-5:][::-1]
        #     # key_frame = int(random.choice(top_k_indices))
        #     key_frame = int(areas.argmax())

        #     # 3. 将视频均匀划分为 N 段
        #     N = self.rvos_sampled_frames
        #     segment_boundaries = np.linspace(0, video_length, N + 1, dtype=int)

        #     # 4. 确定每个 anchor 落在哪个片段
        #     def find_segment(idx):
        #         for i in range(N):
        #             if segment_boundaries[i] <= idx < segment_boundaries[i + 1]:
        #                 return i
        #         return N - 1  # fallback

        #     first_seg = find_segment(first_frame)
        #     key_seg = find_segment(key_frame)

        #     # 5. 保留 anchor，其余段均匀采样1帧
        #     selected_indices = {}

        #     if first_seg == key_seg:
        #         # 同一片段，保留两个帧（放到一个列表里）
        #         selected_indices[first_seg] = sorted(set([first_frame, key_frame]))
        #     else:
        #         selected_indices[first_seg] = [first_frame]
        #         selected_indices[key_seg] = [key_frame]
        ...
        #         else:
        #             rand_idx = start  # degenerate case
        #         selected_indices[i] = [rand_idx]

        #     # 7. 整理所有采样索引
        #     idx_select = []
        #     for indices in selected_indices.values():
        #         idx_select.extend(indices)
        #     idx_select = sorted(set(idx_select))  # 去重 & 排序
            
        #     # print("areas: ", areas)
        #     # print(f"first_frame: {first_frame}, key_frame: {key_frame}")
        #     # print(f"idx_select: {idx_select}")
            
        #     assert areas[key_frame] > areas[first_frame]

        #     if not (first_frame in idx_select and key_frame in idx_select):
        #         print(f"[ERROR] Sampling failed.")
        #         print(f"first_frame: {first_frame}, key_frame: {key_frame}")
        #         print(f"idx_select: {idx_select}")
        #         raise AssertionError("first_frame or key_frame not in sampled indices.")
            
        # else:
        #     idx_select = list(range(video_length))
        ...
        # 以关键帧为中心，进行均匀采样
        if self.rvos_sampled_frames != 1:  
            to_devide = (self.rvos_sampled_frames - 1)
            step_size = math.ceil(video_length / to_devide)
            idx_start = first_valid_idx % step_size
            idx_select = list(range(idx_start, video_length, step_size))  # 以step size为间隔，等距离抽帧
        else:
        ...
        # keyframe_mask = m_final_list[first_valid_idx]     # num_obj, H, W
        sampled_video_frames = [frame_path_list[idx] for idx in idx_select]
        # keyframe = sampled_video_frames[keyframe_id]
        m_final_list = m_final_list[idx_select]        # num_frames, num_obj, H, W
        
        # print(keyframe_id)
        
        # extract bbox of the keyframe
        reference_resize = self.reference_image_resize
        x_factor = reference_resize / width
        y_factor = reference_resize / height
        
        mask_per_frame_dict = dict()
        for num, m_final in enumerate(m_final_list):
            bbox_2d_list = []
            # for mask of each frame
            # print(m_final.shape)
            if np.any(m_final == 1):      # num_obj, H, W
                for obj_mask in m_final:       # H, W
                    if np.all(obj_mask==0):
                        bbox_2d_list.append({"bbox_2d": [0, 0, 0, 0]})
                    else:
                        ...
                        # resize the bbox
                        bbox_2d_list.append({"bbox_2d": [left * x_factor + 0.5, top * y_factor + 0.5, right * x_factor + 0.5, bottom * y_factor + 0.5]})
            else:
                bbox_2d_list = [{"bbox_2d": [0, 0, 0, 0]}]
                
            mask_per_frame_dict[num] = bbox_2d_list
        
        solution = json.dumps(mask_per_frame_dict)
        # print(solution)
        ########## 用来验证bbox是否正确 ##########
        # from PIL import ImageDraw
        # for idx, mask_list in mask_per_frame_dict.items():
        #     resized_frame = Image.open(sampled_video_frames[idx]).convert("RGB").resize((reference_resize, reference_resize), Image.BILINEAR)
        #     draw = ImageDraw.Draw(resized_frame)
        #     for obj_box_list in mask_list:
        #         bbox_2d = obj_box_list["bbox_2d"]
        #         draw.rectangle([(bbox_2d[0], bbox_2d[1]), (bbox_2d[2], bbox_2d[3])], outline="red", width=2)
        #     save_path = os.path.join('/18515601223/Seg-Zero/results_validate', meta_info["video"])
        #     os.makedirs(save_path, exist_ok=True)
        #     resized_frame.save(os.path.join(save_path, str(idx) + "_keyframe_with_bbox.jpg"))
        # print(solution)
        ...
        replace_sent = ", ".join(f'({i}s){"<image>"}' for i in range(video_len))
        ...
        row_dict["video_info"] = (meta_info["video"], meta_info["exp_id"])
        ...
        gt_masks = torch.from_numpy(m_final_list_[frame_ids])      # num_frames, H, W
        resized_masks = gt_masks.unsqueeze(1)     # num_frames, 1, H, W
        resized_masks = F.interpolate(resized_masks, 
                                      size=(reference_resize, reference_resize), 
        ...
        # row_dict["max_mask_area"] = areas[keyframe_id]
