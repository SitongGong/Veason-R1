...
def vision_reasoner_seg_reward(predict_str: str, gt_mask: torch.Tensor=None, seg_model=None, keyframe_id=None, images=None):
    max_accuracy_reward = 0.0
    MAX_OBJECTS = 120  # 设置目标上限
    try: 
        json_match = re.search(r'<answer>\s*(.*?)\s*</answer>', predict_str, re.DOTALL)
        if json_match:
            data = json.loads(json_match.group(1))
            ...
            # pdb.set_trace()
            max_accuracy_reward = compute_iou(masks, gt_mask[keyframe_id].cpu().numpy())
...
def video_match_external_seg_compute_score(predict_str: str, ground_truth: str, gt_mask: torch.Tensor=None, seg_model=None, keyframe_id=None, images=None) -> float:
    ...
    seg_reward = vision_reasoner_seg_reward(predict_str, gt_mask, seg_model, keyframe_id, images)
