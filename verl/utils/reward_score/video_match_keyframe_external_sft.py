...
def vision_reasoner_keyframe_reward(predict_str: str, gt_mask: torch.Tensor, frame_ids=None):
    """
    Args:
    ...
    try:
        ...
        if keyframe_id not in frame_ids:
            return max_keyframe_reward
        else:
            keyframe_id = frame_ids.index(keyframe_id)
...
def vision_reasoner_accuracy_reward(predict_str: str, ground_truth: str, gt_mask: torch.Tensor=None, seg_model=None, images=None, frame_ids=None) -> float:
...
def vision_reasoner_seg_reward(predict_str: str, gt_mask: torch.Tensor=None, seg_model=None, images=None, frame_ids=None) -> float:
    avg_iou_reward = 0.0
    MAX_OBJECTS = 120  # 设置上限
    try:
        # gt_data = json.loads(ground_truth)
...
def video_match_keyframe_sft_external_compute_score(predict_str: str, ground_truth: str, gt_mask: torch.Tensor=None, seg_model=None, keyframe_id=None, images=None, frame_ids=None) -> float:
    # print(predict_str, ground_truth)
    format_reward = vision_reasoner_format_reward(predict_str)
    keyframe_reward = vision_reasoner_keyframe_reward(predict_str, gt_mask, frame_ids)
    accuracy_reward = vision_reasoner_accuracy_reward(predict_str, ground_truth, gt_mask, seg_model, images, frame_ids)
    seg_reward = vision_reasoner_seg_reward(predict_str, gt_mask, seg_model, images, frame_ids)
    
    reward = format_reward + accuracy_reward + seg_reward + keyframe_reward
    return reward