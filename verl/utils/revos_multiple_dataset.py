...
class ReVOSMultiDataset(Dataset):
    """
    ...
    def __init__(
        ...
        self.user_prompt = "Keyframe: <image> " \
                           ...
                           "You are given the Reference Video Frames sampled from a 1 FPS video and one Keyframe, please identify '{Question}' and respond with its bounding box(es) in the Keyframe." \
    ...
    def __getitem__(self, index):
        ...
        # 依次检查每一帧是否包含物体，仅将包含目标的物体作为关键帧
        first_valid_idx = None
        for i in range(m_final_list.shape[0]):
            if np.any(m_final_list[i]):  # 判断这一帧是否包含非零像素（即有目标）
                first_valid_idx = i
                break
        ...
        solution = json.dumps(bbox_2d_list)
        ...
        # resized_frame = Image.open(keyframe).convert("RGB").resize((keyframe_resize, keyframe_resize), Image.BILINEAR)
        # draw = ImageDraw.Draw(resized_frame)
        # for obj_box_list in bbox_2d_list:
        #     bbox_2d = obj_box_list["bbox_2d"]
        #     draw.rectangle([(bbox_2d[0], bbox_2d[1]), (bbox_2d[2], bbox_2d[3])], outline="red", width=2)
        # save_path = os.path.join('Seg-Zero/results_validate', meta_info["video"])
        # os.makedirs(save_path, exist_ok=True)
        # resized_frame.save(os.path.join(save_path, "keyframe_with_bbox.jpg"))
        # print(solution)
        #####################################
        ...
        messages = [
            ...
            {"role": "user", "content": user_prompt.format(Question=exp.lower().strip("."),
                                                           Answer="[{\"bbox_2d\": [x1,y1,x2,y2]}, {\"bbox_2d\": [x1,y1,x2,y2]}]"
                                                           )},
        ]
        ################ New Version ################
        ...
        row_dict["raw_images"] = [Image.open(keyframe).convert("RGB")]
        resized_images = [Image.open(image_path).convert("RGB") for image_path in sampled_video_frames]
        row_dict["raw_images"].extend(resized_images)
