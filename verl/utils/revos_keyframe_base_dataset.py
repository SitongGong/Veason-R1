...
class ReVOS_Keyframe_TS_Dataset(Dataset):
    """
    ...
    def __init__(
        ...
        self.user_prompt =  "Reference Video Frames: <video> " \
                            "You are given {num} reference frames and the query: {Question} " \
                            "1. **Select exactly one keyframe** in which the referred object(s) are most clearly visible (i.e. the frame whose object mask / visible area is maximal). " \
                            "2. **Predict the 2-D bounding box(es)** of all referred object instances on that keyframe. " \
    ...
    def __getitem__(self, index):
        ...
        messages = [
            ...
            {"role": "user", "content": [
                {"type": "text",
                 "text": user_prompt.format(num=str(video_len), 
                         Question=exp.lower().strip("."),
                         Answer="{\"keyframe_id\": N, \"bbox_2d_list\": [[x1,y1,x2,y2], [x1,y1,x2,y2]]}")}
            ] + [{"type": "image", "image": image} for image in sampled_video_frames]
            },
        ]
        ################ New Version ################
